    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (isArena() && isRated() && winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team)
        {
            if (team == winner)
            {
                ...
                winner_arena_team->MemberWon(player, loser_matchmaker_rating, winner_change);
            }
            else
            {
                loser_arena_team->MemberLost(player, winner_matchmaker_rating, loser_change);
    ...
    if (participant) // if the player was a match participant, remove auras, calc rating, update queue
    {
        ...
        if (player)
        {
            ...
            if (isArena())
            {
                ...
                if (isRated() && (GetStatus() == STATUS_IN_PROGRESS || GetStatus() == 2))
                {
                    ...
                    if (winner_arena_team && loser_arena_team && winner_arena_team != loser_arena_team)
                        loser_arena_team->MemberLost(player, GetArenaMatchmakerRating(GetOtherTeam(team)),-80);
					sLog->outArena("Jugador:%u se sale de la arena.", player->GetGUID());
					CharacterDatabase.PExecute("INSERT INTO arenaleave (guid) VALUES (%u)",player->GetGUID());
                }
            }
            if (SendPacket)
            {
        ...
        // removing offline participant
        {
            if (isRated() && (GetStatus() == STATUS_IN_PROGRESS || GetStatus() == 2))
            {
                ...
                if (others_arena_team && players_arena_team)
                    players_arena_team->OfflineMemberLost(guid, GetArenaMatchmakerRating(GetOtherTeam(team)),-80);
            }
        }
