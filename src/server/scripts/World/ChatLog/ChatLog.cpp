...
void ChatLog::_Punish(Player* player, std::string& msg)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_ChatLog_Lexics_Action_Custom))
    {
        m_LexicsCustomAction = LexicsCustomActions(sWorld->getIntConfig(CONFIG_ChatLog_Lexics_Action_Custom_MODE));
        uint32 CastomSpell = sWorld->getIntConfig(CONFIG_ChatLog_Lexics_Action_Custom_Spell);

        switch (m_LexicsCustomAction)
        {
            case LEXICS_ACTION_CUSTOM_SPELL:
                _ApplySpell(player, CastomSpell);
                break;
            default:
                break;
        }
